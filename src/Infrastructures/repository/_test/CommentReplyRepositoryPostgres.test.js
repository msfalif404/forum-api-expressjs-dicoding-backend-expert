...
describe('CommentReplyRepositoryPostgres', () => {
  ...
  describe('getCommentRepliesByCommentIds', () => {
    it('should return empty array when commentIds is empty', async () => {
      // Arrange
      const commentReplyRepositoryPostgres = new CommentReplyRepositoryPostgress(pool, {});

      // Action
      const commentReplies = await commentReplyRepositoryPostgres.getCommentRepliesByCommentIds([]);
    ...
    it('should return empty array when commentIds is null', async () => {
      ...
      // Action
      const commentReplies = await commentReplyRepositoryPostgres.getCommentRepliesByCommentIds(null);
    ...
    it('should return comment replies correctly', async () => {
      // Arrange
      await CommentsTableTestHelper.addComment({ id: 'comment-456', user_id: userId, thread_id: threadId });
      await CommentRepliesTableTestHelper.addCommentReply({
        id: 'reply-333',
        comment_id: commentId,
      });
      ...
      // Action
      const commentReplies = await commentReplyRepositoryPostgres.getCommentRepliesByCommentIds([commentId, 'comment-456']);

      // Assert
      expect(commentReplies).toHaveLength(2);
      expect(commentReplies[0].id).toEqual('reply-333');
      expect(commentReplies[1].id).toEqual('reply-222');
    });
  });
});
