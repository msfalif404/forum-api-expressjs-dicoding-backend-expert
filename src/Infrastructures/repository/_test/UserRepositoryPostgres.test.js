...
describe('UserRepositoryPostgres', () => {
  ...
  describe('getUsersByIds', () => {
    it('should return empty array when userIds is empty', async () => {
      ...
      // Action
      const users = await userRepositoryPostgres.getUsersByIds([]);
    ...
    it('should return empty array when userIds is null', async () => {
      ...
      // Action
      const users = await userRepositoryPostgres.getUsersByIds(null);
    ...
    it('should return users correctly', async () => {
      // Arrange
      await UsersTableTestHelper.addUser({ id: 'user-321', username: 'dicoding1' });
      await UsersTableTestHelper.addUser({ id: 'user-322', username: 'dicoding2' });
      const userRepositoryPostgres = new UserRepositoryPostgres(pool, {});

      // Action
      const users = await userRepositoryPostgres.getUsersByIds(['user-321', 'user-322']);

      // Assert
      expect(users).toHaveLength(2);
      expect(users[0].id).toEqual('user-321');
      expect(users[1].id).toEqual('user-322');
    });
  });
});
